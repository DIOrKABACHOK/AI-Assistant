...
class WeatherView(MethodView):
    ...
    def get(self):
        ...
        city = decrypted_params.get('q')
        if city:
            logger.info(f"Поиск погоды по городу: {city}")
            city_name, lat, lon = self.location_service.geocode_address(city)
            if not all([lat, lon]):
                return jsonify({"error": f"Город '{city}' не найден"}), 404
            weather_data = self.weather_service.get_weather(lat=lat, lon=lon)

        elif decrypted_params.get('lat') and decrypted_params.get('lon'):
            try:
                lat = float(decrypted_params['lat'])
                lon = float(decrypted_params['lon'])
                weather_data = self.weather_service.get_weather(lat=lat, lon=lon)
            except ValueError:
